...
pathStudies <- file.path("./output/studies")
...
lasa  <- list.files(file.path(pathStudies,"lasa/phys-cog"),full.names=T, recursive=T, pattern="out$")
map   <- list.files(file.path(pathStudies,"map/phys-cog"),full.names=T, recursive=T, pattern="out$")
...
octo  <- list.files(file.path(pathStudies,"octo/phys-cog"),full.names=T, recursive=T, pattern="out$")
...
path_model_output <- list_pc[["octo"]][4]
